...
(function(){
  ...
  let _baseJs=[];
  let elfFrame=false;
  if(typeof(window.top.elfFramePage)!=="undefined"&&window.top.elfFramePage.length>0){
    for(let k in window.top.elfFramePage){
      if(window.top.location.pathname.indexOf(window.top.elfFramePage[k])>=0) {
        elfFrame=true;
        break;
      }
    }
  }
  if(window!==window.top.window&&elfFrame){//elfFrame框架子页面
    _baseCss.push("elfFrameSubPage");
    _baseJs=['libs/jquery.min','libs/jquery.cookie','libs/highlight','libs/clipboard','libs/fuse','libs/jquery.smooth','libs/jquery.superHover','libs/jquery.elfAlert','libs/jquery.elfBackground','libs/jquery.pageNumber','libs/jquery.setCache','libs/jquery.elfKeyword','libs/jquery.markIndicate','libs/jquery.slidePage','libs/jquery.elfDrag','libs/jquery.elfRoll','libs/jquery.elfBanner','libs/elfWidget','utils/elf','utils/pageInit'];
  }
  else{
  ...
  if(window===window.top.window&&elfFrame){//elfFrame框架
